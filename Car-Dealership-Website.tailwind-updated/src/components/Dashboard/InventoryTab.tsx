...
type Car = {
  ...
  price: string; year: number; mileage: string; fuel: string
  status: 'Available' | 'Sold' | 'Reserved'; img: string
}
...
const emptyForm = { name: '', brand: '', category: 'Luxury Sedan', price: '', year: new Date().getFullYear(), mileage: '', fuel: 'Petrol', status: 'Available' as const, img: '' }
...
export default function InventoryTab() {
  ...
  const openEdit = (car: Car) => {
    setForm({ name: car.name, brand: car.brand, category: car.category, price: car.price, year: car.year, mileage: car.mileage, fuel: car.fuel, status: car.status, img: car.img })
    setEditCar(car); setShowForm(true)
  }
  const handleSave = () => {
    if (!form.name || !form.brand || !form.price) return
    if (editCar) setCars(prev => prev.map(c => c.id === editCar.id ? { ...form, id: editCar.id } : c))
    else setCars(prev => [...prev, { ...form, id: Date.now() }])
    setShowForm(false)
  }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Search vehicles..."
          className="flex-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E63946] placeholder-gray-400 transition-colors" />
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)}
          className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E63946] cursor-pointer transition-colors">
          {categories.map(c => <option key={c}>{c}</option>)}
        </select>
        <button onClick={openAdd}
          className="bg-[#E63946] hover:bg-[#c62b37] text-white font-bold px-5 py-2.5 rounded-xl text-sm border-none cursor-pointer transition-colors whitespace-nowrap">
          + Add Vehicle
        </button>
      </div>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800/50">
              {filtered.map(car => (
                <tr key={car.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-xs text-gray-500 dark:text-gray-400">{car.category}</td>
                  ...
                  <td className="px-5 py-3.5 text-xs text-gray-500 dark:text-gray-400">{car.fuel}</td>
                  <td className="px-5 py-3.5">
                    <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${statusColor[car.status]}`}>{car.status}</span>
                  </td>
                  <td className="px-5 py-3.5">
            ...
            <div className="p-6 space-y-4">
              {[
                ...
                <div key={f.k}>
                  ...
                  <input type={f.type} value={(form as Record<string, string | number>)[f.k] as string}
                    onChange={set(f.k)} placeholder={f.placeholder}
                    className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#E63946] placeholder-gray-400 transition-colors" />
                </div>
              ))}
              <div className="grid grid-cols-3 gap-3">
                {[
                  { k: 'category', label: 'Category', opts: ['Luxury Sedan', 'Sports Car', 'Luxury SUV', 'Electric', 'Supercar', 'Sports Sedan'] },
                  { k: 'fuel', label: 'Fuel', opts: ['Petrol', 'Diesel', 'Electric', 'Hybrid'] },
                  { k: 'status', label: 'Status', opts: ['Available', 'Sold', 'Reserved'] },
                ].map(f => (
                  <div key={f.k}>
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">{f.label}</label>
                    <select value={(form as unknown as Record<string, string>)[f.k]} onChange={set(f.k)}
                      className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#E63946] cursor-pointer transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 dark:border-gray-800 p-7 text-center">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { setCars(p => p.filter(c => c.id !== deleteId)); setDeleteId(null) }}
